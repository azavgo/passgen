...
pub enum PassgenError {
    FromUtf8Error(str::Utf8Error),
}

impl From<str::Utf8Error> for PassgenError {
...
fn main() ->Result<(), PassgenError> {
    ...
    let password = str::from_utf8(&pass_vec)?;
    println!("Random number from the password array: {}", password); 
    Ok(())
}
